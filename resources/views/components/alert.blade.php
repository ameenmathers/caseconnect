...
<div {{ $attributes->merge(['class' => "flex items-start gap-3 p-4 rounded-lg border {$config['bg']} {$config['border']}"]) }}>
    ...
    @if($dismissible)
        <button type="button" class="{{ $config['text'] }} hover:opacity-75" onclick="this.parentElement.remove()">
